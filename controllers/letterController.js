...
const submitLetter = asyncHandler(async (req, res) => {
    const { fullName, rollNumber, department, semester, recipient, letterType, reason, description, fromDate, endDate } = req.body;

    if (!fullName || !rollNumber || !department || !semester || !recipient || !letterType || !fromDate || !endDate) {
        return res.status(400).json({ error: "All fields are required" });
    }

    // Save Letter Request to DB
    const newLetter = new Letter({ fullName, rollNumber, department, semester, recipient, letterType, reason, description, fromDate, endDate });
    await newLetter.save();

    console.log("Generating AI Letter...");

    // 🔄 Generate AI Letter Content using OpenRouter
    const generatedLetter = await generateAILetter(fullName, rollNumber, department, semester, recipient, letterType, reason, description, fromDate, endDate);
    ...
    res.status(201).json({ message: "Letter generated successfully", generatedLetter });
});

// ✨ AI Letter Generation Function (OpenRouter)
const generateAILetter = async (fullName, rollNumber, department, semester, recipient, letterType, reason, description, fromDate, endDate) => {
    const prompt = `
